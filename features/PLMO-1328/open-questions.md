...
| -------- | ------ | ------------------------------------------------------------------ | ------ |
| Blocking | OQ-001 | "after 12h" trigger — calendar time or 24h cooldown?              | Open   |
| Blocking | OQ-002 | Tooltip state storage — client-side or server-side?                | Open   |
| Blocking | OQ-003 | Tooltip experiment gating — should control-group users see it?    | Open   |
| Blocking | OQ-004 | Tooltip minimum unread threshold — any lead or a minimum count?    | Open   |
| Non-Blocking | OQ-005 | "Liên hệ section" — what exactly is the tooltip anchor/view? | Open |
| Non-Blocking | OQ-006 | Tooltip dismissibility — permanent or daily re-show?           | Open   |
| Non-Blocking | OQ-007 | Yellow toggle conflicts with existing chat color system?          | Open   |
...
| ------ | ------------------------------------------------------ | --------------------------------- | ------------------------------ | ------ |
| OQ-001 | "after 12h" trigger interpretation                     | Tooltip display logic             | Assume 12:00 PM local time     | Open   |
| OQ-002 | Tooltip "shown today" state storage                   | Cross-session / cross-device sync  | Default to client-side storage | Open   |
| OQ-003 | Tooltip experiment gating alignment with Khách tab     | Access control                    | Tooltip hidden for control group | Open   |
| OQ-004 | Minimum unread lead count for tooltip trigger           | UX threshold                      | Show if any unread lead exists | Open   |
...
| ------ | -------------------------------------------- | ----- | ------------------------- | ------------------------------ | ------ |
| OQ-005 | "Liên hệ section" navigation anchor definition | PM   | Tooltip trigger scope     | Assume full Chat/Liên hệ view  | Open   |
| OQ-006 | Tooltip dismissibility — does it re-show next day? | PM | UX behavior | Re-shows on next valid trigger | Open   |
| OQ-007 | Yellow toggle color — conflicts with existing chat system? | Design | Visual design | Use yellow only for Khách toggle | Open   |
...
| ------- | -------------------------------------------------- | ---------- | ------------- |
| A-001   | Tooltip trigger is 12:00 PM local time (not 12h cooldown) | Medium | Wrong trigger timing |
| A-002   | Tooltip state is client-side (localStorage) only  | Medium | Cross-device repeat shows |
| A-003   | Tooltip is gated to experiment treatment group     | Medium | Broken UX for control group |
| A-004   | Tooltip shows for any unread lead count ≥ 1       | High | UX threshold may need adjustment |
